...
## Advanced Protocol Usage & Best Practices

- Always close WebSocket and MQTT connections at the end of the scenario to avoid resource leaks.
- For MQTT/JMS, use `.correlateBy()` or equivalent to match requests and responses reliably.
- Parameterize broker URLs, credentials, and topics/queues for CI/CD and multi-environment support.
- For MQTT/JMS, ensure you have a Gatling Enterprise license; Community Edition does not support these protocols.
- For WebSocket, always authenticate over HTTP before opening the socket if required by your backend.
- Use feeders for device IDs, user IDs, and other dynamic protocol data.

---

## Troubleshooting

- If protocol connections fail, check for missing dependencies or incorrect broker URLs.
- For MQTT/JMS, verify your license and dependency versions.
- Always specify time units explicitly in awaits (e.g., `Duration.ofSeconds(5)`).
- Review scenario logs for missing `.correlateBy()` or unclosed connections.

---

## Maintenance

- Regularly update protocol dependencies and plugins.
- Refactor protocol scenarios as backend or broker APIs evolve.
- Monitor for new protocol support and best practices in Gatling releases.

---

## References

- See [../SKILL.md](../SKILL.md) for the full skill guide and 5-block pattern.
- See [design-patterns.md](design-patterns.md) for modular structure and advanced usage.

---

_Update this guide as new protocol features, patterns, or best practices emerge._
