...
See [reference/design-patterns.md](reference/design-patterns.md) for modular project structure, layering, and code organization in Java, Scala, and TypeScript.

For non-HTTP protocols (WebSocket, MQTT, JMS), see [reference/protocols.md](reference/protocols.md) for setup, dependencies, and DSL usage. Note: MQTT and JMS require Gatling Enterprise.

---

## Advanced Patterns & Best Practices

- Use property-based and randomized feeders for robust scenario coverage.
- Parameterize all environment variables (baseUrl, users, tokens) for CI/CD and cloud runs.
- Use groups, randomSwitch, and doIf for complex user journeys.
- Use `.throttle()` to cap RPS for SLA and compliance testing.
- Integrate with cloud CI runners for distributed or parallel load.
- For long-running tests, use `.circular()` feeders to avoid mid-test failures.
- For APIs with authentication, always extract tokens dynamically per user/session.
- For real-time protocols, always close connections (e.g., `.exec(ws("Close").close())`).

---

## Troubleshooting

- Always validate your project with `scripts/validate.sh` before running.
- Check for missing plugins, wrong file locations, or missing JVM args (Java 21+ needs extra flags).
- For JS/TS, ensure simulation files are in `src/` and named with `.gatling.js`/`.gatling.ts`.
- Review the HTML report for failed requests, assertion breaches, and error details.
- For MQTT/JMS, ensure you have a Gatling Enterprise license.
...
- Refactor simulations and modular layers as APIs and business logic evolve.
- Monitor for new Gatling features, protocol support, and best practices.
- Keep feeders and test data up to date with production-like scenarios.

---

_Update this skill as new Gatling features, protocols, or best practices emerge._
...
- [WebSocket / MQTT / JMS](reference/protocols.md)
- [Modular Design Patterns](reference/design-patterns.md)
